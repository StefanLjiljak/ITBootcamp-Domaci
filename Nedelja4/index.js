...
function addItem() {
    ...
    itemText.innerHTML = `${text} is  ${urgency.value} priority`;
...
let buttonRemSel = document.getElementById('rem-sel');


buttonRemSel.addEventListener('click', checkedRemove);

function checkedRemove() {
    let checky = document.querySelectorAll('.chk-state');
    checky.forEach(i => {
        if (i.checked) i.parentElement.parentElement.remove();
    });
}
...
inverty.addEventListener('click', Invertion);
